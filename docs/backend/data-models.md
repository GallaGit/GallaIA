...
- Existing: SQLAlchemy models and Pydantic schemas for the AgentOS control plane (projects, agents, agent grants, network policy, filesystem ACL, secret refs, tasks, sessions, inbox) plus in-memory AgentOS package schemas under `app/agentos/`.
- Planned: Models/schemas for templates, goals, triggers when those phases start — [ROADMAP.md](../ROADMAP.md).
