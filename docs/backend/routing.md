...
- Existing: `app/api/router.py` aggregates routers (health, projects, agents, tasks, sessions, inbox, agentos); `main.py` includes `api_router` with `prefix="/api/v1"` and keeps unversioned `/` and `/health`. Agent grants: `GET/PUT /agents/{id}/grants` and `/agentos/agents/{name}/grants`. Network policy: `GET/PUT /agents/{id}/network` and `/agentos/agents/{name}/network`.
- Planned: Phase 2+ route modules (filesystem ACL, templates, goals, webhooks) when those phases start — [ROADMAP.md](../ROADMAP.md).
