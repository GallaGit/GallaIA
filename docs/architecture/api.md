...
- Existing:
  ...
  - Control plane: `/api/v1/projects|agents|tasks|sessions|inbox` (SQLite), plus `GET/PUT /agents/{id}/grants`, `GET/PUT /agents/{id}/network`, `GET/PUT /agents/{id}/fs`, and `GET/PUT /agents/{id}/secrets`
  - AgentOS package: `/api/v1/agentos/*` — [CONTRACT.md](../agentos/CONTRACT.md) (includes grants, network policy, filesystem ACL, and secret refs by agent name)
  - OpenAPI at `/docs`
