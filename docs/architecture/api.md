...
- Existing:
  ...
  - Control plane: `/api/v1/projects|agents|tasks|sessions|inbox` (SQLite), plus `GET/PUT /agents/{id}/grants`, `GET/PUT /agents/{id}/network`, and `GET/PUT /agents/{id}/fs`
  - AgentOS package: `/api/v1/agentos/*` — [CONTRACT.md](../agentos/CONTRACT.md) (includes grants, network policy, and filesystem ACL by agent name)
  - OpenAPI at `/docs`
...
- Planned: Phase 2+ endpoints (secret refs, templates, goals, webhooks) only when those phases start — [ROADMAP.md](../ROADMAP.md).
